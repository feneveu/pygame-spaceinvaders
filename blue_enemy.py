...
class BlueEnemy(pg.sprite.Sprite):

    def __init__(self, posx, posy, enemy_group):
        super(BlueEnemy, self).__init__()

        # sets up the images for the blue enemy
        e2_1 = pg.image.load("images/enemy2_1.png").convert_alpha()
        e2_2 = pg.image.load("images/enemy2_2.png").convert_alpha()

        # changes the size of the image
        new_size = (e2_1.get_width() // 6.7, e2_1.get_height() // 6.7)  # Shrinks by 50%

        # creates a list that can toggle between images
        self.img_list = [
            pg.transform.scale(e2_1, new_size),
            pg.transform.scale(e2_2, new_size)
        ]
        ...
        # the blue aliens are worth 10 points
        self.worth = 20
